...
class GuiNormalizeGradient {
  ...
  final Map<double, List<GuiGradientColor>> colormap =
      <double, List<GuiGradientColor>>{};
  ...
  void _normalizeColorMap(Map<double, List<GuiGradientColor>> inColorMap) {
    ...
    Map<double, List<GuiGradientColor>> tmpColorMap =
        <double, List<GuiGradientColor>>{};
    ...
    for (mapStop0Idx = 0;
        (mapStop0Idx < mapStops.length) && (mapStops[mapStop0Idx] != 0);
        mapStop0Idx++) {}
    for (mapStop1Idx = mapStops.length - 1;
        (mapStop1Idx >= 0) && (mapStops[mapStop1Idx] != 1.0);
        mapStop1Idx--) {}
    ...
    for (gradientStop0Idx = 0;
        (gradientStop0Idx < gradientStops.length) &&
            (gradientStops[gradientStop0Idx] != 0);
        gradientStop0Idx++) {}
    for (gradientStop1Idx = gradientStops.length - 1;
        (gradientStop1Idx >= 0) && (gradientStops[gradientStop1Idx] != 1.0);
        gradientStop1Idx--) {}
    ...
    for (double mapstop in mapStops) {
      ...
      for (int i = 0; i < gradientStops.length; i++) {
        effgradient.add(_getGradientColor(
            gradient, gradientStops[i], gradientMinStop, gradientMaxStop));
      }
    ...
    if (bMissing) {
      ...
      for (int i = 0; i < mapStops.length; i++) {
        ...
        double nextstop =
            (i + 1 != mapStops.length) ? mapStops[i + 1] : mapStops.last;
        List<GuiGradientColor>? next = colormap[nextstop];
        if (next == null) {
          nextstop =
              (i + 2 != mapStops.length) ? mapStops[i + 2] : mapStops.last;
          next = colormap[nextstop];
          // in case we still don't have a next
          next ??= _transGradient!;
        }
        List<GuiGradientColor> cur =
            getGradientInBetween(curstop, prev, prevstop, next, nextstop);
  ...
  List<GuiGradientColor> _prepareTransparentGradient() {
    if (_transGradient == null) {
      ...
      for (int i = 0; i < gradientStops.length; i++) {
        _transGradient!.add(_getGradientColor(
            emptyGradient, gradientStops[i], gradientMinStop, gradientMaxStop));
      }
    }
  ...
  Color colorAt(double xStop, double yStop,
      [Color outOfRangeColor = Colors.transparent]) {
    if (yStop < mapMinStop ||
        yStop > mapMaxStop ||
        xStop < gradientMinStop ||
        xStop > gradientMaxStop) return outOfRangeColor;
    // get begin gradient
    int curYStopIdx = 0;
  ...
  static Color getColorInBetween(
      double stop, GuiGradientColor from, GuiGradientColor to) {
    if (stop == from.stop) return from.color;
    if (stop == to.stop) return to.color;
    return Color.lerp(
        from.color, to.color, (stop - from.stop) / (to.stop - from.stop))!;
  }

  // get gradient color from gradient color list based on stop value
  GuiGradientColor _getGradientColor(List<GuiGradientColor> gradient,
      double stop, double minstop, double maxstop) {
    ...
    for (int i = 0; i < gradient.length; i++) {
      ...
      if (curr.stop > stop) {
        return GuiGradientColor(
            Color.lerp(prev.color, curr.color,
                (stop - prev.stop) / (curr.stop - prev.stop))!,
            stop);
      }
      prev = curr;
    }
    ...
    return GuiGradientColor(
        Color.lerp(prev.color, Colors.transparent,
            (stop - prev.stop) / (maxstop - prev.stop))!,
        stop);
  }
...
class GuiGradientImage {
  static Image fromNormalizeGradient(GuiNormalizeGradient gradient, Size size) {
    return gradient.createImage(
        size.width.ceil().toInt(), size.height.ceil().toInt());
  }

  static Image fromRawGradientMap(
      Map<double, List<GuiGradientColor>> colormap, Size size) {
    return GuiNormalizeGradient(colormap)
        .createImage(size.width.ceil().toInt(), size.height.ceil().toInt());
  }
}
