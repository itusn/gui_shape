...
class GuiShapePolar implements IGuiShape {
  ...
  Path getPath({required Size size}) {
    return GeoCustomPoints(getCoordinates(size: size))
        .getPath(
          rotate: GeoAngle.zero,
          clockwise: clockwise,
          size: size, boxFit: boxFit,
  ...
  List<GeoCoordinate2D> getCoordinates({required Size size}) {
    GeoAngle samplingAngle = GeoAngle(radian: (polarEndAngle - polarBeginAngle) / sampling);
    ...
    for (int i = 0; i < sampling; i++) {
      coords.add(ellipse.coordinateByRadian(currentAngle,center,formula(polarAngle) ));
