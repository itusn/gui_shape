...
class GuiShapeCustom implements IGuiShape {
  ...
  GuiShapeCustom({required this.points, required this.rotate, this.clockwise = false, this.boxFit = BoxFit.none, this.cornerRadius = 0}) {
  ...
  Path getPath({required Size size}) {
    return _custom.getPath(rotate: rotate, clockwise: clockwise, size: size, boxFit: boxFit, cornerRadius: cornerRadius);
  }
  ...
  IGuiShape lerpFrom(IGuiShape shape, double t) {
    if (shape is GuiShapeCustom) {
      return GuiShapeCustom(
          ...
          rotate: GeoAngle(radian: lerpDouble(shape.rotate.radian, rotate.radian, t)),
          clockwise: clockwise,
          boxFit: boxFit,
  ...
  IGuiShape lerpTo(IGuiShape shape, double t) {
    if (shape is GuiShapeCustom) {
      return GuiShapeCustom(
          ...
          rotate: GeoAngle(radian: lerpDouble(rotate.radian, shape.rotate.radian, t)),
          clockwise: clockwise,
          boxFit: boxFit,
  ...
  static List<GeoCoordinate2D> _lerp(List<GeoCoordinate2D> from, List<GeoCoordinate2D> to, double t) {
