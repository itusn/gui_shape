...
class GeoStar {
  ...
  late final GeoAngle interiorAngle;      // angle between adjacent vertices w/ origin at the center
  late final GeoAngle halfInteriorAngle;  // half of interior angle
  late final GeoAngle internalAngle;  // angle between adjacent sides of a polygon
  ...
  List<GeoCoordinate2D> getCoordinates({
    ...
    GeoEllipse innerEllipse = GeoEllipse(outerEllipse.xRadius * indentSideFactor, outerEllipse.yRadius * indentSideFactor);
    ...
    for (int i = 0; i < sides; i++) {
      ...
      currentInnerAngle = currentAngle + (clockwise? -halfInteriorAngle.radian : halfInteriorAngle.radian);
  ...
  Path getPath({required GeoAngle startAngle, double indentSideFactor = 0.5, bool clockwise = false, required Size size, BoxFit boxFit = BoxFit.none, double cornerRadius = 0}) {
    List<GeoCoordinate2D> coords = getCoordinates(startAngle: startAngle, indentSideFactor: indentSideFactor, clockwise: clockwise, size: size, boxFit: boxFit);
